...
public class RT_DGCore : MonoBehaviour
{
	int dSize;
	int roomSize;
	int roomSizeDelta;
	...
	List<RT_DGRoomClass> rooms;
	List<RT_DGRoomClass> coridors;
	RT_DGMap dgMap;
	...
	public void Generate()
	{
		...
		int genStep = 0;
		rooms = new List<RT_DGRoomClass>();
		coridors = new List<RT_DGRoomClass>();
		...
		{
			RT_DGRoomClass newRoom = GenerateRandomRoom();
		...
		{//есть хотя бы две комнаты, строим коридоры
			...
			Dictionary<int,bool> network = new Dictionary<int, bool>();
			network.Add(0,true);
			bool allConnectionsMade = false;
			for (int k=0; k<connections.Count; k++) {
				if (network.ContainsKey(k)) {
					List<int> roomConnections = connections[k];
					foreach (int con in roomConnections)
						if (!network.ContainsKey(con))
							network.Add(con,true);
				}
			}
			while (!allConnectionsMade) {
				for(int r=0; r< roomsCount; r++) {
					if (!network.ContainsKey(r)) {
						int[] toRoomClostArray = GetRoomToRoomClosestIndexesArray(r);
						foreach (int room in toRoomClostArray) {
							if (!connections[r].Contains(room) && network.ContainsKey(room)) {
								BuildCoridor(r, room);
								network[r] = true;
								connections[r].Add(room);
								connections[room].Add(r);
								break;
							}
						}
						break;
			...
			RT_DGPointPairClass minMax = GetMinMax();
			dgMap = new RT_DGMap(minMax.point02.GetY() - minMax.point01.GetY(), minMax.point02.GetX() - minMax.point01.GetX(), minMax.point01, minMax.point02);
			foreach (RT_DGRoomClass room in rooms)
			{
				dgMap.FillByRoom(room, false);
			}

			foreach (RT_DGRoomClass coridor in coridors)
			{
	...
	RT_DGPointPairClass GetMinMax()
	{
		RT_DGPointPairClass toReturn = new RT_DGPointPairClass();
		RT_DGPointClass rMin = rooms[0].GetCorner(0);
		RT_DGPointClass rMax = rooms[0].GetCorner(2);
		toReturn.point01 = new RT_DGPointClass(rMin);
		toReturn.point02 = new RT_DGPointClass(rMax);
		foreach(RT_DGRoomClass room in rooms)
		{
	...
	{
		RT_DGRoomClass startRoom = rooms[startIndex];
		RT_DGRoomClass endRoom = rooms[endIndex];
		...
		RT_DGRoomClass newCoridor01 = new RT_DGRoomClass(new RT_DGPointClass(clostV, sPos - coridorThicknes / 2), new RT_DGPointClass(ePos + (Sign(ePos - clostV) > 0 ? coridorThicknes - coridorThicknes / 2 : -1 * coridorThicknes / 2), sPos - coridorThicknes / 2 + coridorThicknes));
		RT_DGRoomClass newCoridor02 = new RT_DGRoomClass(new RT_DGPointClass(ePos - coridorThicknes / 2, clostH), new RT_DGPointClass(ePos - coridorThicknes / 2 + coridorThicknes, sPos + (Sign(sPos - clostH) > 0 ? coridorThicknes - coridorThicknes / 2 : -1 * coridorThicknes / 2)));
	...
	RT_DGRoomClass GenerateRandomRoom()
	{
		...
		return new RT_DGRoomClass(cx, cy, w, h);
	}
	...
	void SetColor(int id, GameObject go)
	{
		...
		{
			go.GetComponent<Renderer>().sharedMaterial.color = RT_DRCustomEditor.HSVToRGB(RT_DRCustomEditor.GetSin(id, 0f), 0.5f, 0.8f);
		}
	}
